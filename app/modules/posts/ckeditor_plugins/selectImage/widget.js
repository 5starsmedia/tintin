	...
	// 		└──────┴───────────────────────────────┴─────────────────────────────┘
	//
	// Non-captioned widget (config.SelectImages_alignClasses defined)
	// 		┌──────┬───────────────────────────────┬─────────────────────────────┐
	// 		│Align │Internal form                  │Data                         │
	// 		├──────┼───────────────────────────────┼─────────────────────────────┤
	// 		│none  │<wrapper>                      │<img />                      │
	...
	// 		└──────┴────────────────────────────────────────┴────────────────────────────────────────┘
	//
	// Captioned widget (config.SelectImages_alignClasses defined)
	// 		┌──────┬────────────────────────────────────────┬────────────────────────────────────────┐
	// 		│Align │Internal form                           │Data                                    │
	// 		├──────┼────────────────────────────────────────┼────────────────────────────────────────┤
	// 		│none  │<wrapper>                               │<figure />                              │
	...
	// @returns {Object}
export function widgetDef( editor ) {
		var alignClasses = editor.config.SelectImages_alignClasses,
			captionedClass = editor.config.SelectImages_captionedClass;
		...
		function inflate() {
			...
			if ( this.deflated ) {
				this.widget = editor.widgets.initOn( this.element, 'SelectImages', this.widget.data );
        ...
        if (this.data.hasPreview) {
          this.parts.image.setAttribute('data-image-with-preview', true);
        } else {
          this.parts.image.removeAttribute('data-image-with-preview');
        }
			...
			init: function() {
				var helpers = CKEDITOR.plugins.SelectImages,
					image = this.parts.image,
					data = {
						hasCaption: !!this.parts.caption,
						hasPreview: image.getAttribute( 'data-image-with-preview' ) == 'true',
				...
				this.wrapper[ ( data.hasPreview ? 'add' : 'remove' ) + 'Class' ]( 'cke_image_withpreview' );
			...
			// Overrides default method to handle internal mutability of SelectImages.
			...
			getClasses: ( function() {
				...
				return function() {
					...
					// Neither config.SelectImages_captionedClass nor config.SelectImages_alignClasses
	...
	CKEDITOR.plugins.SelectImages = {
		stateShifter: function( editor ) {
			// Tag name used for centering non-captioned widgets.
			var doc = editor.document,
				alignClasses = editor.config.SelectImages_alignClasses,
				captionedClass = editor.config.SelectImages_captionedClass,
				editable = editor.editable(),

				// The order that stateActions get executed. It matters!
				shiftables = [ 'hasCaption', 'hasPreview', 'align', 'link' ];
			...
			var stateActions = {
				...
				hasPreview:	function( shift, oldValue, newValue ) {
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							captionPlaceholder: shift.newData.alt || 'Caption'//editor.lang.SelectImages.captionPlaceholder
						} ), doc );
    ...
    if ( widget.data.hasPreview) {
      wrapper.addClass('cke_image_withpreview');
    } else {
      wrapper.removeClass('cke_image_withpreview');
    }
		if ( alignClasses ) {
	...
	// <figure class="{config.SelectImages_captionedClass}"> elements.
	//
	...
	function downcastWidgetElement( editor ) {
		...
		return function( el ) {
			...
			// inline styles or classes (SelectImages_alignClasses).
	...
	function centerWrapperChecker( editor ) {
		var captionedClass = editor.config.SelectImages_captionedClass,
			alignClasses = editor.config.SelectImages_alignClasses,
		...
		return function( el ) {
			...
			// Centering wrapper got to be... centering. If SelectImages_alignClasses are defined,
	...
	function setupResizer( widget ) {
		...
		//resizer.setAttribute( 'title', editor.lang.SelectImages.resizer );
...
export	function alignCommandIntegrator( editor ) {
		var execCallbacks = [],
			enabled;
...
export
	...
	// on configurations like config#SelectImages_alignClasses or
	// config#SelectImages_captionedClass.
	//
	// @param {CKEDITOR.editor}
	// @returns {Object}
	function getWidgetAllowedContent( editor ) {
		var alignClasses = editor.config.SelectImages_alignClasses,
			rules = {
				...
				figure: {
					classes: '!' + editor.config.SelectImages_captionedClass
				},
				figcaption: true
			};
 ...
 *		CKEDITOR.config.SelectImages_captionedClass = 'captionedImage';
 *
 * @cfg {String} [SelectImages_captionedClass='image']
 * @member CKEDITOR.config
 */
CKEDITOR.config.SelectImages_captionedClass = 'image';
 ...
 *
 *		config.SelectImages_alignClasses = [ 'align-left', 'align-center', 'align-right' ];
 *
 ...
 *
 * @since 4.4
 * @cfg {String[]} [SelectImages_alignClasses=null]
 * @member CKEDITOR.config
 */
