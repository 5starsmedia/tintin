    ...
    angular.module('ngEditableTree', ['ngResource'])
        ...
        .factory('ngNestedResource', ['$resource', '$q', '$rootScope', function ($resource, $q, $rootScope) {
            function ResourceFactory(url, paramDefaults, actions, options) {
                var defaultActions = {
                    update: { method: 'PUT' },
                    create: { method: 'POST', params: { 'insert': true } },
                ...
                resource.prototype.$moveItem = function (before, position, cb) {
                    ...
                    if (position == 0) {
                      clone.parentId = before._id;
                    } else {
                      clone.parentId = before.parentId;
                    }
                    return clone.$save({ position: position + 1 }, function (item) {
                        cb(item);
                    });
                };
                resource.getTree = function (data, cb) {
                    ...
                    var def = $q.defer();
                    resource.getAsTree(data, function (result) {
